...
using TechCos_LRG.JLG.JLGUserControl;
...
namespace TechCos_LRG
{
    public partial class JLGLoan : KryptonForm
    {
        ...
        public MaterialButton borBtn, memBtn, amtBtn, expBtn, presanBtn, duediliBtn, visitRepBtn;
        JLGDetails jlgBor;
        public JLGmem1 jlgMem1;
        public memberpageNum membpageNum;
        public JLGAmount jLGAmount;
        public Panel memBottomPanel;
        public PreSanctionSurvey preSanctionSurvey;
        public VisitReport visitReport;
        public DueDiligenceReport dueDiligenceReport;
        SqlConnection cn;
        ...
        public JLGLoan()
        {
            ...
            expBtn = exportBtn;
            presanBtn = preSanctionSurveyBtn;
            duediliBtn = dueDiligenceReportBtn;
            visitRepBtn = visitReportBtn;
            memBottomPanel = bottomMemPgPanel;
            jlgBor = new JLGDetails();
            jlgMem1 = new JLGmem1();
            membpageNum = new memberpageNum();
            jLGAmount = new JLGAmount();
            preSanctionSurvey = new PreSanctionSurvey();
            dueDiligenceReport = new DueDiligenceReport();
            visitReport = new VisitReport();
            AddContUserControl(jlgBor);
            memberDetailsBtn.Enabled = false;
            amountDetailsBtn.Enabled = false;
            preSanctionSurveyBtn.Enabled = false;
            dueDiligenceReportBtn.Enabled = false;
            visitReportBtn.Enabled = false;
        ...
        {
            double per;
            int month = int.Parse(JLGAmount.instance.noOfMonths.Text);
            double rate;
            rate = (double.Parse(JLGAmount.instance.mclr.Text) + JLGAmount.instance.getSpread()) / 100;
            int amt = int.Parse(JLGAmount.instance.noOfMonths.Text);
        ...
        private void preSanctionSurveyBtn_Click(object sender, EventArgs e)
        {
            AddContUserControl(preSanctionSurvey);
            bottomMemPgPanel.Visible = false;
        }

        private void dueDiligenceReportBtn_Click(object sender, EventArgs e)
        {
            AddContUserControl(dueDiligenceReport);
            bottomMemPgPanel.Visible = false;
        }

        private void visitReportBtn_Click(object sender, EventArgs e)
        {
            AddContUserControl(visitReport);
            bottomMemPgPanel.Visible = false;
        }
    }
}
